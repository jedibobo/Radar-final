...
from multicam_thread import Multicam, concat_imgs
...
def test_video(video_path):
    ...
    while True:
        ...
        fgmask = mask_wide.get_mask(img_wide)  # GMM get fmask for moving objects
        fgmask =mask_left.get_mask(img_left)

        img_wide = unpack_results(
            results[0], img_wide, GMMmask=mask_wide, WITH_GMM=True)
        img_left = unpack_results(
            results[1], img_left, GMMmask=mask_left, WITH_GMM=True)
